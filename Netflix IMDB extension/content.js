...
(() => {
  ...
  const PLATFORM_CONFIGS = {
    netflix: {
      ...
      extractTitle: (element, selectors) => {
        ...
        if (linkWithAriaLabel) {
          const ariaLabel = linkWithAriaLabel
    ...
    prime: {
      ...
      cardSelectors: ["article[data-testid='card']"], // new selector
      titleSelectors: ["[data-card-title]"], // get from attribute
      imageContainerSelectors: ["article[data-testid='card']"],

      extractTitle: (element, selectors) => {
        // First try attribute "data-card-title"
        const attrTitle = element.getAttribute("data-card-title");
        if (attrTitle) {
          return { title: attrTitle.trim(), type: null };
        }

        // Fallback: loop over selectors
        for (const selector of selectors) {
          const el = element.querySelector(selector);
          if (!el) continue;
          const title =
  ...
  const TitleExtractor = {
    extract(element, platformConfig) {
      const result = platformConfig.extractTitle(
        element,
        platformConfig.titleSelectors
      );
      ...
      if (!result) {
        ...
        if (hostname.includes("netflix.com")) {
          console.log(
            "Available aria-label links:",
            element.querySelectorAll("a[aria-label]")
          );
          console.log(
            "Available fallback text:",
            element.querySelectorAll(".fallback-text")
          );
        }
      }
    ...
    findBestMatch(searchTitle, results, expectedType = null) {
      ...
      if (expectedType) {
        const typeFiltered = results.filter((result) => {
          const resultType =
            result.titleType?.toLowerCase() || result.type?.toLowerCase();
  ...
  const ApiService = {
    ...
    async cleanExpiredEntries() {
      ...
      for (const [key, entry] of Object.entries(this.cache)) {
        if (
          !entry.timestamp ||
          now - entry.timestamp > BASE_CONFIG.CACHE_MAX_AGE
        ) {
    ...
    async getRating(titleData) {
      ...
      if (
        this.cache[cacheKey] &&
        this.isCacheEntryValid(this.cache[cacheKey])
      ) {
    ...
    async processQueue() {
      ...
      while (
        ...
        this.activeRequests < BASE_CONFIG.MAX_CONCURRENT_REQUESTS
      ) {
    ...
    async fetchFromApi(title, expectedType, cacheKey, retryCount = 0) {
      try {
        ...
        if (data?.titles?.length > 0) {
          const bestMatch = FuzzyMatcher.findBestMatch(
            title,
            data.titles,
            expectedType
          );
          ...
          if (bestMatch && bestMatch.result.rating?.aggregateRating > 0) {
            const rating = {
              ...
              matchedTitle:
                bestMatch.result.primaryTitle || bestMatch.result.title,
  ...
  const StreamingRatings = {
    ...
    async init() {
      ...
      console.log(
        `IMDB Ratings Extension initialized for ${this.platform.config.name}`
      );
    ...
    findCards() {
      ...
      for (const selector of this.platform.config.cardSelectors) {
        ...
        if (foundCards.length > 0) {
          if (
            ...
            console.log(
              `Found ${foundCards.length} Hotstar cards with selector: ${selector}`
            );
          } else if (hostname.includes("netflix.com")) {
            console.log(
              `Found ${foundCards.length} Netflix cards with selector: ${selector}`
            );
          }
        }
      }
      ...
      const filteredCards = cards.filter(
        (card) => !card.querySelector(".imdb-rating-overlay")
      );

      console.log(
        `${hostname} - Total cards found: ${cards.length}, After filtering: ${filteredCards.length}`
      );
